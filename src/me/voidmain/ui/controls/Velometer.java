...
public class Velometer extends View {
	...
	protected static final int INVALID_LEVEL = -1;
	...
	protected int calcAngle(MotionEvent event) {
		float diffX = event.getX() - mStartX;
		float diffY = event.getY() - mStartY;
	...
	protected void updateCurPosition(MotionEvent event) {
		mCurX = mMeasuredWidth / 2 - mStartX + event.getX();
		mCurY = mMeasuredHeight / 2 - mStartY + event.getY();
